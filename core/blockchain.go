...
func NewBlockChain(chainDb ethdb.Database, config *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	fmt.Printf("blockchain NewBlockChain %v \n config %v \n engine %v \n",chainDb,config,engine)
...
func (bc *BlockChain) getProcInterrupt() bool {
	fmt.Printf("blockchain getProcInterrupt \n ")
...
func (bc *BlockChain) loadLastState() error {
	fmt.Printf("blockchain loadLastState \n ")
...
func (bc *BlockChain) SetHead(head uint64) error {
	fmt.Printf("blockchain SetHead head %v \n",head)
...
func (bc *BlockChain) FastSyncCommitHead(hash common.Hash) error {
	fmt.Printf("blockchain FastSyncCommitHead hash %v \n",hash)
...
func (bc *BlockChain) GasLimit() *big.Int {
	fmt.Printf("blockchain GasLimit \n")
	bc.mu.RLock()
	defer bc.mu.RUnlock()
...
func (bc *BlockChain) LastBlockHash() common.Hash {
	fmt.Printf("blockchain LastBlockHash\n")
	bc.mu.RLock()
	defer bc.mu.RUnlock()
...
func (bc *BlockChain) CurrentBlock() *types.Block {
	fmt.Printf("blockchain CurrentBlock \n")
	bc.mu.RLock()
	defer bc.mu.RUnlock()
...
func (bc *BlockChain) CurrentFastBlock() *types.Block {
	fmt.Printf("blockchain CurrentFastBlock \n")
	bc.mu.RLock()
	defer bc.mu.RUnlock()
...
func (bc *BlockChain) Status() (td *big.Int, currentBlock common.Hash, genesisBlock common.Hash) {
	fmt.Printf("blockchain Status td %v \n currentBlock %v \n genesisBlock %v \n",td,currentBlock,genesisBlock)
	bc.mu.RLock()
	defer bc.mu.RUnlock()
...
func (bc *BlockChain) SetProcessor(processor Processor) {
	fmt.Printf("blockchain SetProcessor processord %v \n",processor)
	bc.procmu.Lock()
...
func (bc *BlockChain) SetValidator(validator Validator) {
	fmt.Printf("blockchain SetValidator validator %v \n",validator)
	bc.procmu.Lock()
	defer bc.procmu.Unlock()
...
func (bc *BlockChain) Validator() Validator {
	fmt.Printf("blockchain Validator \n ")
	bc.procmu.RLock()
...
func (bc *BlockChain) Processor() Processor {
	fmt.Printf("blockchain Processor \n ")
	bc.procmu.RLock()
	defer bc.procmu.RUnlock()
	return bc.processor
}
...
func (bc *BlockChain) StateAt(root common.Hash) (*state.StateDB, error) {
	fmt.Printf("blockchain StateAt root %v \n",root)
	return state.New(root, bc.stateCache)
}

// Reset purges the entire blockchain, restoring it to its genesis state.
func (bc *BlockChain) Reset() error {
	fmt.Printf("blockchain Reset \n")
...
func (bc *BlockChain) ResetWithGenesisBlock(genesis *types.Block) error {
	fmt.Printf("blockchain ResetWithGenesisBlock genesis %v \n",genesis)
...
func (bc *BlockChain) Export(w io.Writer) error {
	fmt.Printf("blockchain Export w %v \n",w)
	return bc.ExportN(w, uint64(0), bc.currentBlock.NumberU64())
}

// ExportN writes a subset of the active chain to the given writer.
func (bc *BlockChain) ExportN(w io.Writer, first uint64, last uint64) error {
	fmt.Printf("blockchain ExportN w %v \n first %v \n last %v \n",w,first,last)
	bc.mu.RLock()
	defer bc.mu.RUnlock()
...
func (bc *BlockChain) insert(block *types.Block) {
	fmt.Printf("blockchain insert block %v \n",block)
...
func (bc *BlockChain) GetBody(hash common.Hash) *types.Body {
	fmt.Printf("blockchain GetBody hash %v \n",hash)
...
func (bc *BlockChain) GetBodyRLP(hash common.Hash) rlp.RawValue {
	fmt.Printf("blockchain GetBodyRLP hash %v \n",hash)
	// Short circuit if the body's already in the cache, retrieve otherwise
...
func (bc *BlockChain) HasBlock(hash common.Hash, number uint64) bool {
	fmt.Printf("blockchain HasBlock hash %v \n number %v \n",hash,number)
...
func (bc *BlockChain) HasBlockAndState(hash common.Hash) bool {
	fmt.Printf("blockchain HasBlockAndState hash %v \n ",hash)
...
func (bc *BlockChain) GetBlock(hash common.Hash, number uint64) *types.Block {
	fmt.Printf("blockchain GetBlock hash %v \n number %v \n",hash,number)
...
func (bc *BlockChain) GetBlockByHash(hash common.Hash) *types.Block {
	fmt.Printf("blockchain GetBlockByHash hash %v ",hash)
...
func (bc *BlockChain) GetBlockByNumber(number uint64) *types.Block {
	fmt.Printf("blockchain GetBlockByNumber number %v ",number)
...
func (bc *BlockChain) GetBlocksFromHash(hash common.Hash, n int) (blocks []*types.Block) {
	fmt.Printf("blockchain GetBlocksFromHash hash %v \n n %v \n",hash,n)
...
func (bc *BlockChain) GetUnclesInChain(block *types.Block, length int) []*types.Header {
	fmt.Printf("blockchain GetUnclesInChain block %v \n length %v \n",block,length)
	uncles := []*types.Header{}
...
func (bc *BlockChain) procFutureBlocks() {
	fmt.Printf("blockchain procFutureBlocks")
...
func SetReceiptsData(config *params.ChainConfig, block *types.Block, receipts types.Receipts) {
	fmt.Printf("blockchain SetReceiptsData config %v \n block %v \n receipts %v \n",config,block,receipts)
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	fmt.Printf("blockchain InsertReceiptChain blockChain %v \n receiptChain %v \n",blockChain,receiptChain)
	bc.wg.Add(1)
	defer bc.wg.Done()
...
func (bc *BlockChain) WriteBlockAndState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	fmt.Printf("blockchain WriteBlockAndState block %v \n",block)
	bc.wg.Add(1)
	defer bc.wg.Done()
...
func (bc *BlockChain) InsertChain(chain types.Blocks) (int, error) {
	fmt.Printf("blockchain InsertChain \n")
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	fmt.Printf("blockchain insertChain len(chain) %v \n",len(chain))
	// Do a sanity check that the provided chain is actually ordered and linked
...
func (st *insertStats) report(chain []*types.Block, index int) {
	fmt.Printf("blockchain report st %v \n chain %v \n index %v \n",st,chain,index)
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	fmt.Printf("blockchain reorg oldBlock %v \n newBlock %v \n",oldBlock,newBlock)
	var (
...
func (bc *BlockChain) PostChainEvents(events []interface{}, logs []*types.Log) {
	fmt.Printf("blockchain PostChainEvents events")
...
func (bc *BlockChain) BadBlocks() ([]BadBlockArgs, error) {
	fmt.Printf("blockchain BadBlocks \n")
...
func (bc *BlockChain) addBadBlock(block *types.Block) {
	fmt.Printf("blockchain addBadBlock block %v \n",block)
	bc.badBlocks.Add(block.Header().Hash(), block.Header())
}

// reportBlock logs a bad block error.
func (bc *BlockChain) reportBlock(block *types.Block, receipts types.Receipts, err error) {
	fmt.Printf("blockchain reportBlock block %v \n receipts %v \n",block,receipts)
...
func (bc *BlockChain) InsertHeaderChain(chain []*types.Header, checkFreq int) (int, error) {
	fmt.Printf("blockchain InsertHeaderChain chain %v \n checkFreq %v \n",chain,checkFreq)
	start := time.Now()
...
func (bc *BlockChain) writeHeader(header *types.Header) error {
	fmt.Printf("blockchain writeHeader header %v \n",header)
	bc.wg.Add(1)
	defer bc.wg.Done()
...
func (bc *BlockChain) CurrentHeader() *types.Header {
	fmt.Printf("blockchain CurrentHeader\n",)
	bc.mu.RLock()
	defer bc.mu.RUnlock()
